...
describe('burgerConstructorSlice', () => {
  ...
  describe('reducers', () => {
    ...
    it('should handle addFilling', () => {
      ...
      const secondAction =
        burgerConstructorSlice.actions.addFilling(mockFilling);
    ...
    it('should handle removeFilling', () => {
      ...
      const stateWithFilling = burgerConstructorSlice.reducer(
        initialState,
        addAction
      );
      ...
      const removeAction =
        burgerConstructorSlice.actions.removeFilling(fillingKey);
    ...
    it('should handle move filling items', () => {
      ...
      expect(state.filling.map((i) => i._id)).toEqual([
        'filling-0',
        'filling-1',
        'filling-2',
      ]);

      const moveAction = burgerConstructorSlice.actions.move({
        dragIndex: 0,
        hoverIndex: 1,
      });
      const movedState = burgerConstructorSlice.reducer(state, moveAction);

      expect(movedState.filling.map((i) => i._id)).toEqual([
        'filling-1',
        'filling-0',
        'filling-2',
      ]);
