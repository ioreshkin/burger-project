...
const LoginPage = () => {
    ...
    const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        dispatch(fetchLogin({email: email, password: password})).then(() => {
            navigate(from ? from : '/');
        }).catch(err => {
            console.log(err);
        });
        ...
        <div className={styles.container}>
            <div className={styles.content_wrap}>
                ...
                <p className={`${styles.block} text text_type_main-default text_color_inactive mt-20 mb-4`}>
                    Вы - новый пользователь? <Link to='/register' className={styles.link} replace={false}>Зарегистрироваться</Link>
                </p>
                <p className={`${styles.block} text text_type_main-default text_color_inactive`}>
