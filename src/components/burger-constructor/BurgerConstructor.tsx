...
const BurgerConstructor = () => {
  ...
  const handleClick = async () => {
    if (user.name === '') {
      ...
      dispatch(fetchOrder({ ingredients: ids })).then(() => dispatch(reset()));
    }
  };
  ...
  return (
    <section
      ...
      className="mt-25 pr-4 pl-4"
      ref={ref}
    >
      <div
        className={`${styles.external_container} mb-4 pr-8`}
      ...
      <div
        className={`${styles.scroll_container} custom-scroll`}
        data-testid="constructor-fillings"
      >
        {filling.map((ingredient, index) => {
          return (
            <BurgerConstructorDraggableItem
      ...
      <div
        className={`${styles.external_container} mt-4 mb-10 pr-8`}
      ...
      <div className={styles.pay}>
        <div className={`${styles.price} mr-10`}>
          <p data-testid="order-total" className="text text_type_digits-medium">
            {cost}
          </p>
